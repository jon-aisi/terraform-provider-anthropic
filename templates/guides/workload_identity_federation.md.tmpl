...
- **Trust and access changes are in place.** An issuer's `issuer_url` and `jwks`, and a rule's `match`, `target`, `oauth_scope`, `workspace_id` and `applies_to_all_workspaces`, are all updatable. A change to any of them is a single `~` line in the plan, and the provider adds a warning next to it, but it changes who can mint tokens, what they act as and where, for every rule involved, the moment it applies. Review those lines as access changes. The issuer the bootstrap rule of section 2 depends on should carry `lifecycle { prevent_destroy = true }`.
